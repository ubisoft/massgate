...
#include "stdafx.h"
#include "MMG_Tiger.h"
// This is the reference implementation of Tiger, wrapped in a class.
// Looks like shit, but the encryptioncommunity can't code.

#undef OPTIMIZE_FOR_ALPHA
...
/* This is the official definition of TIGER_ROUND */
...
// MMG_Tiger implementation below

MMG_CryptoHash 
MMG_Tiger::GenerateHash(const void* theData, unsigned long theDataLength) const
{ 
	MC_PROFILER_BEGIN(profa, __FUNCTION__);
	word64 res[3];
	tiger((const word64*)theData, theDataLength, res);
...
void
MMG_Tiger::Continue(const char* theData, unsigned long theDataLength)
{
	...
	{
		const unsigned int nBytes = __min(64-myDataLeftoversLength, theDataLength);
	...
	{
		tiger_compress((const word64*)myDataLeftovers, myState);
		myDataLeftoversLength = 0;
	}

	word64 i;
	for (i=theDataLength; i >= 64; i-=64)
	{
		tiger_compress((const word64*)theData, myState);
		theData += 64;
	}
	// If theData has any leftovers (i.e. the last <64 bytes left, then store them and use them later.
	if (i)
	{
		const word64 nBytes = __min(i, 64-word64(myDataLeftoversLength));
		memcpy(myDataLeftovers + myDataLeftoversLength, theData, size_t(nBytes));
		i -= nBytes;
		myDataLeftoversLength += size_t(nBytes);
		assert(i==0);
	}
}
...
MMG_Tiger::End()
{
	...
	if(j>56)
	{
		for(; j<64; j++)
			temp[j] = 0;
		tiger_compress(((word64*)temp), myState);
		j=0;
	}
...
MMG_Tiger::TestTiger()
{
	for (unsigned int BUFSIZE = 1023; BUFSIZE < 1024*1024*20; BUFSIZE += rand()*102)
	{
		...
		for (unsigned int i = 0; i < BUFSIZE; i++)
			buff[i] = rand();
		MMG_Tiger tiger;
		MMG_CryptoHash singleHash = tiger.GenerateHash(buff, BUFSIZE);

		tiger.Start();
		for (unsigned int i = 0; i < BUFSIZE;)
		{
			const unsigned int rndBytes = unsigned int(rand());
			const unsigned int nBytes = __min(BUFSIZE - i, rndBytes);
			tiger.Continue(buff + i, nBytes);
			i += nBytes;
		}
		delete[]buff;
		MMG_CryptoHash multiHash = tiger.End();
